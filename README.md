...
- `/broadcast_offer` отправляет offer-рассылку с персональной URL-кнопкой `🟢 Купить за 99 ₽`; каждому получателю создаётся отдельный Robokassa intent на 50 распознаваний.
...
- показывает HTML-preview с будущей кнопкой `🟢 Купить за 99 ₽`;
- предлагает аудиторию `all`, `free` или `no_balance`;
- перед отправкой показывает confirm с количеством получателей;
- для каждого получателя создаёт персональный `ExternalPaymentIntent` `founder_offer` на 99 ₽ и 50 распознаваний;
- кнопка ведёт на персональную ссылку, которая фиксирует `offer_broadcast_clicked_at` и сразу перенаправляет в Robokassa;
- успешная отправка фиксирует `offer_broadcast_sent_at`;
- blocked users и другие Telegram API ошибки логируются и не останавливают рассылку.

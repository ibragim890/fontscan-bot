...
- Дополнительная оплата картой через Робокассу, если включена в переменных окружения.
- Цены, лимиты подписок и trial-настройки можно менять из Telegram без Railway redeploy.
- Оплата Stars запускается сразу при нажатии `Оплатить Designer Stars` или `Оплатить Studio Stars`.
...
Оплата картой добавляется как второй способ оплаты и не заменяет Telegram Stars.
...
- Пока рублёвая цена равна цене в Stars: `price_rub = price_stars`.
...
8. Нажмите `Оплатить Designer Stars` или `Оплатить Studio Stars`: invoice создаётся сразу.
9. При `ROBOKASSA_ENABLED=true` нажмите `Оплатить Designer картой` или `Оплатить Studio картой`: бот должен отправить URL-кнопку Робокассы.
10. Проверьте `/status`: должен отобразиться тариф, дата окончания и остаток лимита.
11. Проверьте `/cancel`: продление отменяется, доступ остаётся до даты окончания.

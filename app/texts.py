...
DEFAULT_BOT_TEXTS: dict[str, tuple[str, str]] = {
    "main_menu": (
        "Главное меню",
        """Привет. Это бот для поиска шрифтов по изображению.

Загрузи картинку с текстом — я проанализирую её и покажу похожий шрифт.
...
Designer — {price_designer} ₽ / {limit_designer} распознаваний
Studio — {price_studio} ₽ / {limit_studio} распознаваний""",
    ),
    "font_result_found": (
        "Результат: найден",
        """Шрифт: {font_name}
...
Это наиболее похожий вариант. Бот может допускать неточное определение шрифта.""",
    ...
    "font_result_not_found": (
        "Результат: не найден",
        """Текст на изображении плохо читается.

Для точного поиска лучше отправить картинку, где буквы крупные, чёткие и без размытия.""",
    ),
...
FORCED_TEXT_UPDATE_KEY = "bot_texts_card_payments_v3"
